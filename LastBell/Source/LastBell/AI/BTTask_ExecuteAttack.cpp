    ...
    UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
    ...
    EBoxingMove MoveToExecute = AttackMove;

    if (bPickRandomMove)
    {
        float Total = MoveWeights.X + MoveWeights.Y + MoveWeights.Z;
        float Rand = FMath::FRandRange(0.f, Total);
        if (Rand < MoveWeights.X)
            MoveToExecute = EBoxingMove::Jab;
        else if (Rand < MoveWeights.X + MoveWeights.Y)
            MoveToExecute = EBoxingMove::Hook;
        else
            MoveToExecute = EBoxingMove::Uppercut;
    }

    Boxer->AIExecuteAttack(MoveToExecute);
